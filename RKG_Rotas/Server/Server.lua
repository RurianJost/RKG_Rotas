	...
	if user_id then
		...
		for k,v in pairs(Config.GiveItem) do
			...
			elseif vRP.hasPermission(user_id, Config.Permissions.Tequila) or vRP.hasPermission(user_id, Config.Permissions.Vanilla) then
				if v == Config.GiveItem.Tequila then
					local escolheritem = math.random(#Config.GiveItem.Tequila)
					local nomeitem = Config.GiveItem.Tequila[escolheritem]
					local quantityitem = Config.GiveItem.Tequila.Quantity
					vRP.giveInventoryItem(user_id,nomeitem,math.random(quantityitem[1],quantityitem[2]))
				end
